...
// phiInfo.js - Updated version with Direct Firebase Sync

// Remove IIFE since this is now a module
// (function () { 

const PHI_INFO_KEY = "phi_info_v1"; // Kept for short-key logic if needed, but main data is in FireStore
const SHORT_KEY_INSPECTOR = "phi_info_inspector";
const SHORT_KEY_AREA = "phi_info_area";
const SHORT_KEY_PHOTO = "phi_info_photo";
const MAX_FILE_BYTES = 10 * 1024 * 1024;

let inMemoryData = []; // Local cache of data from Firebase
...
// Debugging
console.log("[phiInfo] module loaded.");



let phiInfoUnsub = null;
let subscriptionActive = false;

function startPhiInfoSubscription() {
  if (phiInfoUnsub) return;

  console.log("[phiInfo] Starting subscription to 'info'...");

  // Subscribe to Firebase Data
  phiInfoUnsub = DirectFirebaseService.subscribe('info', (data) => {
    subscriptionActive = true;
    console.log("[phiInfo] Data received:", data ? "Present" : "Null/Empty");

    // Check if data changed
    const newData = data || [];
    // Only update if different? Or just always update.
    inMemoryData = newData;

    // Update short keys / header immediately
    updateShortKeys(inMemoryData);
    updateHeaderInfo(inMemoryData[0]);

    // Attempt to refresh UI if visible
    const container = document.querySelector(".phi-info-container");
    if (container && container.parentNode) {
      // We need to re-render, but renderPhiInfo needs the container. 
      // We can try to find the ID or pass the parent.
      renderPhiInfo(container.parentNode);
    }
  });
}
...
onAuthStateChanged(auth, (user) => {
  if (user) {
    console.log("[phiInfo] User authenticated:", user.uid);
    startPhiInfoSubscription();
  } else {
    console.log("[phiInfo] User signed out.");
    if (phiInfoUnsub) {
      phiInfoUnsub();
      phiInfoUnsub = null;
    }
...
function saveAll(v) {
  DirectFirebaseService.save('info', v);
}

function updateShortKeys(v) {
  try {
    const latest = (v && v.length) ? v[0] : null;

    // Save to global state instead of localStorage
    window.PHIInfo = window.PHIInfo || {};
    window.PHIInfo.currentData = latest;

    // dispatch event so other modules can react immediately
    window.dispatchEvent(new CustomEvent("phiInfoUpdated", {
      detail: latest // Send the actual object
    }));
  } catch (e) {
...
function renderPhiInfo(containerOrId) {
          ...
          <div>
            <div class="phi-info-form-grid" style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
              ...
              <div>
                ...
                <select id="phi_role" style="${baseInputStyle}">
                  <option value="PHI">PHI</option>
                  <option value="SPHI">SPHI</option>
                  <option value="SPHID">SPHID</option>
                  <option value="MOH">MOH</option>
                  <option value="Visitor">Visitor</option>
                </select>
                <div id="container_role_visitor" style="display:none; margin-top:6px;">
                   <select id="phi_role_visitor" style="${baseInputStyle}">
                      <option value="PHNS">PHNS</option>
                      <option value="PHM">PHM</option>
                      <option value="SPHM">SPHM</option>
                      <option value="AMOH">AMOH</option>
                   </select>
                </div>
              </div>
  ...
  // NEW REFS for Role
  const roleSelect = container.querySelector("#phi_role");
  const areaInput = container.querySelector("#phi_area"); // Ref for Area field
  const visitorContainer = container.querySelector("#container_role_visitor");
  const visitorSelect = container.querySelector("#phi_role_visitor");
  const dependentFields = container.querySelector("#role_dependent_fields");
  ...
  function updateRoleUI() {
    ...
    if (visitorContainer) visitorContainer.style.display = isVisitor ? 'block' : 'none';
    if (dependentFields) dependentFields.style.display = isVisitor ? 'none' : 'block';
    updateAreaLogic();
  }

  function getEffectiveRole() {
    const main = roleSelect.value;
    if (main === 'Visitor') return visitorSelect.value;
    return main;
  }

  function updateAreaLogic() {
    ...
    // Logic:
    // 1. PHI, PHM -> Editable
    // 2. SPHM, PHNS, MOH, AMOH, SPHI -> Readonly, Show MOH Name
    // 3. SPHID -> Readonly, Show RDHS Name (or NIHS Kalutara)

    if (['PHI', 'PHM'].includes(role)) {
      areaEl.readOnly = false;
      areaEl.style.background = '#fff';
      areaEl.style.cursor = 'text';
      // Don't auto-change value, let user type
    } else if (['SPHM', 'PHNS', 'MOH', 'AMOH', 'SPHI'].includes(role)) {
      areaEl.readOnly = true;
      areaEl.style.background = '#f9f9f9';
      areaEl.style.cursor = 'default';

      // Get MOH Name
      let mohName = "";
      if (authTypeSelect.value === 'Local') {
        mohName = mohInput.value;
      } else {
        mohName = mohSelect.value;
      }
      areaEl.value = mohName;
      ...
      // Get RDHS/NIHS Name
      let authName = "";
      if (authTypeSelect.value === 'NIHS') {
        authName = "NIHS Kalutara";
      } else {
        // If Local or RDHS, usually RDHS name for SPHID context? 
        // User said: "SPHID සඳහා අදාළ දිස්ත්‍රික්කයේ(RDHS ) නම පෙන්වන්න. NIHS නම් NIHS Kalutara..."
        // If Local, maybe not applicable for SPHID? Assuming RDHS select value if RDHS.
        if (authTypeSelect.value === 'RDHS') {
          authName = rdhsSelect.value;
        } else {
          authName = rdhsSelect.value; // Fallback to RDHS select even if logic weird
        }
      }
      areaEl.value = authName;
    }
  }
  ...
  if (latest) {
    ...
    // Role Init
    const r = latest.role || 'PHI';
    const subRoles = ['PHNS', 'PHM', 'SPHM', 'AMOH'];
    if (subRoles.includes(r)) {
      roleSelect.value = 'Visitor';
      visitorSelect.value = r;
    } else {
      roleSelect.value = r;
      // Handle case where r is 'Visitor' (from older save?) -> default sub role
      if (r === 'Visitor') {
        roleSelect.value = 'Visitor';
        // keep default PHNS
      }
    ...
    updateRoleUI();
    // Actually updateAreaLogic relies on values in DOM.
    // Ensure MOH/RDHS values set first (they are set above in latest block).
    // So calling it here is safe.
    setTimeout(updateAreaLogic, 50); // Small delay to ensure all selects populated/set
  ...
  visitorSelect.addEventListener("change", updateAreaLogic);
  ...
  mohSelect.addEventListener("change", updateAreaLogic);
  localInput.addEventListener("input", updateAreaLogic);
  ...
  container.querySelector("#phi_save").addEventListener("click", function () {
    ...
    function persist() {
      ...
      localStorage.setItem("phi_info_area", area);
...
window.PHIInfo.getData = function () {
  // Return the first record (latest) as that's what the app commonly uses
  return inMemoryData && inMemoryData.length ? inMemoryData[0] : (window.PHIInfo.currentData || null);
};
window.PHIInfo.getAll = function () {
  return inMemoryData;
}

// ensure key exists
// LocalStorage legacy key usage removed.
// We now rely on DirectFirebaseService and window.PHIInfo.currentData
